...
const Presets = (function() {
    ...
    const PRESETS_STORAGE_KEY = 'ai_meas_presets_v4';
    const LEGACY_STORAGE_KEY = 'ai_meas_presets'; // Used in old version? Actually, the old version used cookies per field. 
    // In the task doc, it says "especially pay attention to ai_meas_presets and ai_measurment_3_3_presets.json".
    // Wait, the old main.js used cookies: `getCookie(key)`, but maybe the user added presets manually later.
    // I will implement a robust preset system that works seamlessly.

    // Default presets (if nothing saved)
    const defaultPresets = {
        min: {
            add_mm: false, scale: '1:1', font_name: 'default', line_len: 5, precis: 0, line_stroke: 0.1, gap: 0.5, line_indent: 0.5, arrow_width: 2,
            font_size: 8, cyan: 0, magenta: 0, yellow: 0, black: 100, add_layer: false, layer_name_text: 'layout', out_artboard: false
        },
        med: {
            add_mm: false, scale: '1:1', font_name: 'default', line_len: 10, precis: 2, line_stroke: 0.2, gap: 1, line_indent: 1, arrow_width: 3.5,
            font_size: 14, cyan: 0, magenta: 0, yellow: 0, black: 100, add_layer: false, layer_name_text: 'layout', out_artboard: false
        },
        max: {
            add_mm: false, scale: '1:1', font_name: 'default', line_len: 20, precis: 3, line_stroke: 0.5, gap: 2, line_indent: 2, arrow_width: 5,
    ...
    const defaultGlobalState = {
        add_mm: false, scale: '1:1', unit_select: 'mm', font_name: 'default', line_len: 10, precis: 2, line_stroke: 0.2, gap: 0.5, line_indent: 1.0, arrow_width: 3.5,
        font_size: 14, cyan: 0, magenta: 0, yellow: 0, black: 100, add_layer: false, layer_name_text: 'layout', out_artboard: false
    };
    ...
    function init() {
        ...
        if (saved) {
            ...
            // Attempt legacy migration or use default
            const legacy = Storage.get(LEGACY_STORAGE_KEY);
            if (legacy) {
                currentPresets = legacy;
    ...
    function collectCurrentSettings() {
        return {
            ...
            font_name: document.getElementById('font_name') ? document.getElementById('font_name').value : 'default',
    ...
    function applySettings(p) {
        ...
        if (p.font_name !== undefined) {
            if (typeof FontPicker !== 'undefined' && typeof FontPicker.setFontByName === 'function') {
                FontPicker.setFontByName(p.font_name);
            } else if (document.getElementById('font_name')) {
                document.getElementById('font_name').value = p.font_name;
            }
        }
    ...
    function handlePresetClick(type) {
        ...
        if (isSaveMode) {
            ...
            Storage.set(PRESETS_STORAGE_KEY, currentPresets);
            // Optional: document.getElementById('chk_save').checked = false;
        } else {
    ...
    // Expose for main initialization (e.g. loading last used state)
    function loadLastUsed() {
        // If we want to restore last state
        const lastUsed = Storage.get('ai_meas_last_state');
        if (lastUsed) applySettings(lastUsed);
    }

    function saveLastUsed() {
        Storage.set('ai_meas_last_state', collectCurrentSettings());
    }
